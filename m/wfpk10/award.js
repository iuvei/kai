...
$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('http://m.qx66.com/wfpk10/' + _page, { t: Math.random() }, function (text) {
    ...
    function loadAwardTimes() {
        $.post('../../wfpk10/getPk10AwardTimes.do', {t: Math.random() }, function (data) {
            if(data.current.awardNumbers!='') {
                $('.newIssue span').html(data.current.periodNumber1.substr(2));
                $('.nextIssue span').html(data.next.periodNumberStr.substr(2));
...
function getHistoryData(count,date) {
    $.get("../../wfpk10/getHistoryData.do", { count:count,date:date,t: Math.random() }, function (result) {
        if(result&&result.rows.toString().length>3){
        	...
        	for(var i in result.rows){
                ...
                html += '<div class="qihao">'+'<div>'+'<span class="Issue">'+data.termNum.substr(2) +'</span>'+'期'+'</div>'+'<div>'+ data.lotteryTime.substring(10, 16)+'</div>'+'</div>';
...
function getPkData(date) {
    $.get("../../wfpk10/getHistoryData.do", {date: date, page: 1, offset: 15, t: Math.random()}, function (result) {
        if (result.count) {
            loadData(result.count);
            loadpage(date,'wfpk10');
        }
    }, "json");
