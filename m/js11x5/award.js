$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('http://m.qx66.com/js11x5/' + _page, { t: Math.random() }, function (text) {
...
function getHistoryData(count,date) {
    $.get("../../js11x5/getHistoryData.do", { count:count,date:date,t: Math.random() }, function (result) {
...
function getPkData(date) {
    $.get("../../js11x5/getHistoryData.do", {date: date, page: 1, offset: 15, t: Math.random()}, function (result) {
        if (result.count) {
            loadData(result.count);
            loadpage(date,'js11x5');
        }
    }, "json");
