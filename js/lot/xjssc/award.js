$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('xjssc/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('xjssc/getxjsscAwardData.do', {  t: Math.random() }, function (data) {
            requireCount += 1;
    ...
    function loadAwardTimes() {
        $.get('xjssc/getxjsscAwardTimes.do', { ajaxhandler: 'GetxjsscAwardTimes', t: Math.random() }, function (data) {
            //请求到数据后需要做的事情
            ...
            var leavePeriod = 96 - cpNumber;
            if (leavePeriod == 0) {
                var d = new Date();
                var nd = new Date(data.next.awardTime.split(' ')[0].replace("-", "/", "gi"));
                if (d.getDate() == nd.getDate()) leavePeriod = 96;
            }
...
function updateHistoryRecord() {
    $.get('xjssc/getNewestRecord.do', { ajaxhandler: 'GetNewestRecord', t: Math.random() }, function (data) {
...
function LoadxjsscTipSet() {
    var tip_twoball = parseInt($.cookie("xjssc_tip_twoball") == null ? 1 : $.cookie("xjssc_tip_twoball"));
    if (1 == tip_twoball) {
