$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('tcssc/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('tcssc/getCqsscAwardData.do', {  t: Math.random() }, function (data) {
            requireCount += 1;
    ...
    function loadAwardTimes() {
        $.get('tcssc/getCqsscAwardTimes.do', { t: Math.random() }, function (data) {
            ...
            var leavePeriod = 1152 - (cpNumber.substr(cpNumber.length-4));

            if (leavePeriod == 0) {
                var d = new Date();
                var nd = new Date(data.next.awardTime.split(' ')[0].replace("-", "/", "gi"));
                if (d.getDate() == nd.getDate()) leavePeriod = 1152;
            }
...
function updateHistoryRecord() {
    $.get('tcssc/getNewestRecord.do', { t: Math.random() }, function (data) {
