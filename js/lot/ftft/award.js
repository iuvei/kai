...
$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('xyft/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('xyft/getPk10AwardData.do', { ajaxhandler: 'GetPk10AwardData', t: Math.random() }, function (data) {
    ...
    function loadAwardTimes() {
        $.get('xyft/getPk10AwardTimes.do', { ajaxhandler: 'GetPk10AwardTimes', t: Math.random() }, function (data) {
            if(data.current.awardNumbers!='') {
                var nums;
                var str = "";
                nums = data.current.awardNumbers.split(',');
...
function getHistoryData(count) {
    $.get("xyft/getHistoryData.do", { count:typeof(count)=="undefined"?16:count,t: Math.random() }, function (result) {
        if(result&&result.rows){
            ...
            for(var i in result.rows){
                ...
                html += '<tr class="' + clsName + '"><td><p class="">' +  data.termNum.substr(4)  + '</p></td><td><p class="">'
...
function updateHistoryRecord() {
    $.get('xyft/getNewestRecord.do', { ajaxhandler: 'GetNewestRecord', t: Math.random() }, function (data) {
