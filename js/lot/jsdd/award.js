$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('js28/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('js28/getCqsscAwardData.do', {  t: Math.random() }, function (data) {
            var str = "";
    ...
    function loadAwardTimes() {
        console.log(2)
        $.get('jsdd/getCqsscAwardTimes.do', { t: Math.random() }, function (data) {
            var str = "";
            if (data.current.awardNumbers != ''){
                var totalNum_a =data.current.awardNumbers.split(',')[0];
                var totalNum_b =data.current.awardNumbers.split(',')[1];
...
//alert(showCountDown(cpNextAwardTimeInterval, data.next.periodNumber));
   ...
   // window.setTimeout(awardTick, 1000);
    ...
    function polling() {
        $.post('jsdd/getCqsscAwardTimes.do', {t: Math.random()}, function (data) {
            if(data.status == 2){
                return
            }
            if(data.current.awardNumbers==''){
...
function updateHistoryRecord() {
    $.get('jsdd/getNewestRecord.do', { t: Math.random() }, function (data) {
