$(function () {
    ...
    function afterAwarded() {
        ...
        if (_page) {
            ...
            setTimeout(function () {
                $.get('jnd28/' + _page, { t: Math.random() }, function (text) {
    ...
    var awardTick = function () {
        $.get('jnd28/getCqsscAwardData.do', {  t: Math.random() }, function (data) {
            var str = "";
    ...
    function loadAwardTimes() {
        $.get('jnd28/getCqsscAwardTimes.do', { t: Math.random() }, function (data) {
            var str = "";
            if (data.current.awardNumbers != ''){
                var nums = data.current.awardNumbers.split(',');
                var totalNum_a = Number(nums[1]) + Number(nums[4]) + Number(nums[7]) +Number(nums[10]) + Number(nums[13]) + Number(nums[16]);
...
//alert(showCountDown(cpNextAwardTimeInterval, data.next.periodNumber));
   ...
   // window.setTimeout(awardTick, 1000);
    ...
    function polling() {
        $.post('jnd28/getCqsscAwardTimes.do', {t: Math.random()}, function (data) {
            if(data.status == 2){
                return
            }
            if(data.current.awardNumbers==''){
...
function updateHistoryRecord() {
    $.get('jnd28/getNewestRecord.do', { t: Math.random() }, function (data) {
