...
function loadAjax() {
    $.ajax({
        ...
        success: function(data) {
            ...
            var opentime_remaining = data.datas.present.opentime_remaining;
            var last_opencode=data.datas.last_opencode.expect;
            var last_opencode1=data.datas.present.expect;
            nextopent = data.datas.present.opentime_remaining;
            //记录上期开奖期数
           ...
           if(last_expect1 == last_opencode1) {
               ...
               nextopenTime(data.datas.present.opentimestamp) // 底部栏的倒计时
               ...
               clearTimeout(typeSix);
                var ma = data.datas.last_opencode.opencode.split(",");
                var kaijiArr = [];
...
function dateAndissue(data) {
    var nextdate = data.datas.present.opentimestamp.slice(0, 10).replace("-", '年').replace("-", "月") + "号";
    $("#data_b").text(nextdate);
    $("#issue_b").text(data.datas.present.expect);
    $("#issue_c").text(data.datas.last_opencode.expect)
