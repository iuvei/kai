...
function loadAjax() {
	$.ajax({
		...
		success: function(data) {
			var n,e;
			n = data.last_opencode.opencode.split(","); //开奖结果
			e = data.last_opencode.opencode.split(","); //开奖结果
			x = data.present.expect;//下期开奖期号
			b = data.last_opencode.expect;//本期开奖期号
			cutTime = data.present.opentime_remaining;//开奖倒计时
			fcsdv.stopVid(n,e);
		},
	});
...
let b = $(window).height()/$('.content').height();
		$('.content').css('transform','scale('+n+','+b+')');
