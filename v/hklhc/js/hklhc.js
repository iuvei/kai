...
$("#try").click(function(e) { //试试手气
    ...
    var eachnone = setTimeout(function() {
        ...
        $("#die").css("cssText","background-image:url(img/each.gif) !important");
...
//检查是否提交请求
function setApiurl() {
    setStorage('token', 'cface3be804efda16b3ba7d8ec028e1b');
    setStorage('host', 'http://api.dk138.vip');
...
function init() {
    //重新设置上期的开奖期数状态【true:重新记录在last_expect】
     ...
     last_expect_status1 = true;
    setApiurl();
}

//请求后台开奖数据
var typeSix, typetwo, nextopent;
var errcount = 0,
     last_expect = '',//记录上期开奖的期数
     last_expect1 = '',//记录上期开奖的期数
    apiurl='';
...
function loadAjax() {
    $.ajax({
        ...
        success: function(data) {
            ...
            var opentime_remaining = data.present.opentime_remaining;
            var last_opencode=data.last_opencode.expect;
            var last_opencode1=data.present.expect;
            nextopent = data.present.opentime_remaining;
            //记录上期开奖期数
            ...
            //记录上期开奖期数
            if (last_expect_status1) {
                last_expect1 = last_opencode1;
                last_expect_status1 = false;
            }

           if(last_expect1 == last_opencode1) {
               clearInterval(addinter);
               stopanimate();
               addloadhtml(data); // 加载开奖号码
               nextopenTime(data.present.opentimestamp) // 底部栏的倒计时

            }if(opentime_remaining<50&&opentime_remaining>0) {
                bgSound.play();
                originalHtml = $(".number>ul").html();
                $(".number>ul").html("");
               ...
               clearTimeout(typeSix);
                var ma = data.last_opencode.opencode.split(",");
                var kaijiArr = [];
...
function dateAndissue(data) {
    var nextdate = data.present.opentimestamp.slice(0, 10).replace("-", '年').replace("-", "月") + "号";
    $("#data_b").text(nextdate);
    $("#issue_b").text(data.present.expect);
    $("#issue_c").text(data.last_opencode.expect)
